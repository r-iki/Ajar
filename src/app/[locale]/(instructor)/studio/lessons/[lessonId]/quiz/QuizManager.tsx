            ...
            <div className="p-8 space-y-6">
               ...
               <div className="grid gap-6 sm:grid-cols-2">
                   <div className="space-y-2">
                     ...
                     <textarea 
                       defaultValue={getLangVal(q.question, "id")}
                       onBlur={(e) => handleUpdateQuestion(q.id, { question: { ...q.question, id: e.target.value } })}
                  ...
                  <div className="space-y-2">
                     ...
                     <textarea 
                       defaultValue={getLangVal(q.question, "en")}
                       onBlur={(e) => handleUpdateQuestion(q.id, { question: { ...q.question, en: e.target.value } })}
                       className="w-full rounded-2xl border bg-background/50 px-5 py-3 text-sm font-medium focus:ring-2 focus:ring-amber-500/20 outline-hidden transition-all resize-none"
                     />
                  </div>
               </div>
                       ...
                       <div key={choice.id} className={`flex items-center gap-4 rounded-2xl border p-4 transition-all ${
                         choice.isCorrect ? 'border-emerald-500/50 bg-emerald-500/5 shadow-inner' : 'bg-background/30'
                          ...
                          <div className="grid flex-1 gap-4 sm:grid-cols-2">
                             <input 
                               defaultValue={getLangVal(choice.text, "id")}
                               onBlur={(e) => handleUpdateChoice(choice.id, { text: { ...choice.text, id: e.target.value } })}
                               className="bg-transparent text-sm font-medium outline-hidden"
                               placeholder="Pilihan (ID)"
                             />
                             <input 
                               defaultValue={getLangVal(choice.text, "en")}
                               onBlur={(e) => handleUpdateChoice(choice.id, { text: { ...choice.text, en: e.target.value } })}
                               className="bg-transparent text-sm font-medium outline-hidden"
