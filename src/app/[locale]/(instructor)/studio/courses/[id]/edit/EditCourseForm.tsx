...
export function EditCourseForm({ course, categories: initialCategories, locale }: { course: any, categories: any[], locale: string }) {
  ...
  const handleCreateCategory = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        router.refresh();
        // Since router.refresh() is async and might take time, we can't easily get the new cat here 
        // without another fetch. But the user asked for it "here". 
        // I'll suggest they refresh or I can try to fetch updated categories.
      } else {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        toast.error(result.error || "Gagal memperbarui kursus.");
      }
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-8 sm:grid-cols-2">
        <div className="space-y-3">
          <label className="text-xs font-black uppercase tracking-widest text-muted-foreground">Harga (IDR)</label>
          <input 
            name="price"
            type="number"
