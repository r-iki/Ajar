...
import { updateCourse, createCategory } from "@/actions/course";
...
export function EditCourseForm({ 
  ...
  const statusOptions = [
    { value: "draft", label: t("draftCourses") },
    { value: "published", label: t("publishedCourses") },
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                ...
                >
                  <Plus className="size-3" /> + {tCommon("add")}
                </button>
                <Link 
        ...
        <div className="grid gap-8 sm:grid-cols-2">
          <div className="space-y-3">
            <label className="text-xs font-black uppercase tracking-widest text-muted-foreground">{t("price")}</label>
            <div className="relative">
              <span className="absolute left-4 top-1/2 -translate-y-1/2 text-xs font-black text-muted-foreground font-mono">Rp</span>
              <input 
                name="price"
                type="number"
                defaultValue={course.price}
                required
                className="w-full rounded-2xl border border-border bg-card pl-11 pr-5 py-4 text-sm font-bold text-foreground focus:ring-2 focus:ring-primary/20 outline-hidden transition-all shadow-2xs font-mono"
              />
            </div>
          </div>
          <div className="space-y-3">
            <label className="text-xs font-black uppercase tracking-widest text-muted-foreground">{t("status")}</label>
            <CustomSelect
              name="status"
              value={status}
              onChange={setStatus}
