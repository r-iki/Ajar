  ...
  return (
    <div className="space-y-12 pb-20">
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        {[
          { label: t("totalCourses"), value: stats.totalCourses, icon: BookOpen, color: "text-blue-500", bg: "bg-blue-500/10" },
          { label: t("totalStudents"), value: stats.totalStudents, icon: Users, color: "text-emerald-500", bg: "bg-emerald-500/10" },
          { label: t("enrollments"), value: stats.totalEnrollments, icon: GraduationCap, color: "text-amber-500", bg: "bg-amber-500/10" },
          { label: t("revenue"), value: `Rp ${stats.totalRevenue.toLocaleString()}`, icon: DollarSign, color: "text-purple-500", bg: "bg-purple-500/10" },
        ].map((item, i) => (
      ...
      <div className="grid gap-8 lg:grid-cols-3">
         ...
         <section className="lg:col-span-2 space-y-6">
            <div className="flex items-center justify-between border-b pb-4">
               <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-lg font-black uppercase tracking-tight">{t("coursePerformance")}</h2>
               </div>
            </div>
               ...
               <div className="divide-y">
                  {performance.slice(0, 5).map((course) => (
                     <div key={course.id} className="group flex items-center justify-between p-6 transition-colors hover:bg-muted/50">
                        <div className="space-y-1">
                           <p className="font-black group-hover:text-primary transition-colors">{tDb(course.title, locale)}</p>
                           <div className="flex items-center gap-3 text-[10px] font-black text-muted-foreground uppercase tracking-widest">
                              <span className="flex items-center gap-1">
                                 <Users className="size-3" />
                                 {t("studentsEnrolled", { count: course.enrollmentCount })}
                              </span>
                           </div>
                        </div>
                        <Link 
         ...
         <aside className="space-y-6">
            <h2 className="text-lg font-black uppercase tracking-tight">{t("instructorInfo")}</h2>
            <div className="rounded-[2.5rem] bg-gradient-to-br from-indigo-600 to-violet-700 p-8 text-white shadow-xl shadow-indigo-500/20">
               <div className="size-12 rounded-2xl bg-white/20 flex items-center justify-center mb-6 backdrop-blur-md">
                  <TrendingUp className="size-6" />
               </div>
               <p className="text-sm font-bold opacity-80">{t("insightTitle")}</p>
               <h3 className="mt-2 text-2xl font-black leading-tight">{t("insightHeadline")}</h3>
               <p className="mt-4 text-xs opacity-60 leading-relaxed">
                  {t("insightDesc")}
               </p>
               <Link 
                  href={`/${locale}/studio/courses/new` as any}
                  className="mt-8 block text-center w-full rounded-2xl bg-white text-indigo-600 py-4 text-sm font-black transition-all hover:scale-[1.02] active:scale-95 shadow-lg"
               >
                  {t("newCourseBtn")}
               </Link>
            </div>
              ...
              return (
                <div key={course.id} className="group relative flex flex-col overflow-hidden rounded-[2.5rem] border bg-card transition-all hover:shadow-2xl hover:shadow-primary/5">
                  <div className="relative aspect-video w-full overflow-hidden">
                    {course.thumbnail ? (
                      <Image src={course.thumbnail} alt={tDb(course.title, locale)} fill className="object-cover transition-transform duration-700 group-hover:scale-110" />
                    ) : (
