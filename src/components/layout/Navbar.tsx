...
export function Navbar() {
  ...
  if (session) {
    ...
    navLinks.push({ href: "/dashboard", label: tNav("dashboard") });
    if (user.role === "instructor" || user.role === "admin") {
      navLinks.push({ href: "/studio/courses", label: tNav("studio") });
    }
    if (user.role === "admin") {
      navLinks.push({ href: "/overview", label: tNav("admin") });
    }
  }
  ...
  return (
    <header className={`sticky top-0 w-full border-b glass transition-all ${isMenuOpen ? "z-[100]" : "z-50"}`}>
      <div className="mx-auto flex w-full max-w-6xl items-center justify-between px-4 py-4">
        <Link href="/" className="flex items-center gap-2.5 text-xl font-black tracking-tight text-primary transition-transform hover:scale-105">
          <Image
            src="/favicon.jpg"
            alt="Ajar Logo"
            width={32}
            ...
            {!isPending && (
              <>
                {session ? (
                  ...
                  <Link 
                    href="/sign-in" 
                    className="rounded-full bg-primary px-5 py-2 text-sm font-bold text-primary-foreground transition-all hover:opacity-90 active:scale-95 shadow-md shadow-primary/20"
                  >
                    {tNav("signIn")}
                  </Link>
                )}
              ...
              {session && (
                <>
                  <Link 
                    href="/settings"
                    onClick={() => setIsMenuOpen(false)}
                    className={`flex items-center gap-3 p-4 rounded-2xl border transition-all ${pathname.includes('/settings') ? "bg-primary text-primary-foreground border-primary" : "bg-muted/30 hover:bg-muted"}`}
                  >
                    <Settings className="size-5" />
                    <span className="font-bold">{tNav("settings")}</span>
                  </Link>
                  <button 
                    onClick={() => {
                      handleSignOut();
