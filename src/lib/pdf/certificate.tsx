      ...
      <Page size="A4" orientation="landscape" style={styles.curriculumPage}>
                ...
                return (
                    <View key={module.id} style={styles.moduleItem} wrap={false}>
                        <View style={styles.moduleTitleRow}>
                            <Text style={styles.moduleTitle}>{idx + 1}. {module.titleId}</Text>
                            <Text style={styles.moduleTime}>{formatMinutes(moduleTime)}</Text>
                        </View>
                        <View style={styles.lessonList}>
                            {module.lessons.map((lesson) => (
                                <Text key={lesson.id} style={styles.lessonItem}>• {lesson.titleId} ({lesson.duration}m)</Text>
                            ))}
                        </View>
                    </View>
                );
        ...
        <View style={{ marginTop: "auto", borderTop: "1pt solid #e2e8f0", paddingTop: 8 }}>
            <Text style={{ fontSize: 7, color: "#94a3b8", textAlign: "center" }}>
                Halaman ini adalah lampiran resmi dari sertifikat ID {certificateCode}. Verifikasi keaslian dokumen dapat dilakukan di ajar.id/id/v/{certificateCode}.
            </Text>
        </View>
      </Page>
    </Document>
